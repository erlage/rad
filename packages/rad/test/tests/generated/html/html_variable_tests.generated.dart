...
void html_variable_test() {
  group('HTML Variable tests:', () {
    RT_AppRunner? app;
    ...
    test('should set id', () async {
      await app!.buildChildren(
        widgets: [
          Variable(key: Key('some-key-1'), id: 'some-id-1'),
          Variable(key: Key('some-key-2'), id: 'some-id-2'),
          Variable(key: Key('some-key-3'), id: 'some-id-3'),
        ],
    ...
    test('should clear attribute "classes" if updated value is null', () async {
      ...
      await app!.updateChildren(
        widgets: [
          Variable(key: Key('el-1'), className: null),
        ],
        updateType: UpdateType.setState,
        parentRenderElement: app!.appRenderElement,
      );
    ...
    test('should set attribute "hidden" only if its true', () async {
      await app!.buildChildren(
        widgets: [
          Variable(key: Key('el-1'), hidden: false),
          Variable(key: Key('el-2'), hidden: null),
          Variable(key: Key('el-3'), hidden: true),
        ],
        parentRenderElement: app!.appRenderElement,
      );
    ...
    test('should clear attribute "hidden" if updated value is not true',
      ...
      await app!.buildChildren(
        widgets: [
          Variable(key: Key('el-1'), hidden: true),
          Variable(key: Key('el-2'), hidden: true),
          Variable(key: Key('el-3'), hidden: true),
          Variable(key: Key('el-4'), hidden: true),
        ],
        parentRenderElement: app!.appRenderElement,
      );
      ...
      await app!.updateChildren(
        widgets: [
          Variable(key: Key('el-1'), hidden: true),
          Variable(key: Key('el-2'), hidden: false),
          Variable(key: Key('el-3'), hidden: null),
          Variable(key: Key('el-4')),
        ],
        updateType: UpdateType.setState,
        parentRenderElement: app!.appRenderElement,
      );
    ...
    test('should set "click" event listener only if provided', () async {
      ...
      await app!.buildChildren(
        widgets: [
          Variable(key: Key('el-1')),
          Variable(key: Key('el-2'), onClick: null),
          Variable(key: Key('el-3'), onClick: listener),
        ],
        parentRenderElement: app!.appRenderElement,
      );
    ...
    test('should clear "click" event listener', () async {
      ...
      await app!.buildChildren(
        widgets: [
          Variable(key: Key('el-1')),
          Variable(key: Key('el-2'), onClick: listener),
        ],
        parentRenderElement: app!.appRenderElement,
      );
    ...
    test('should set style', () async {
      await app!.buildChildren(
        widgets: [
          Variable(key: Key('widget-1'), style: 'some style'),
          Variable(key: Key('widget-2'), style: 'some "messy" style'),
          Variable(key: Key('widget-3'), style: "some 'messy' style"),
        ],
        parentRenderElement: app!.appRenderElement,
      );
    ...
    test('should set title', () async {
      await app!.buildChildren(
        widgets: [
          Variable(key: Key('widget-1'), title: 'some title'),
          Variable(key: Key('widget-2'), title: 'some "messy" title'),
          Variable(key: Key('widget-3'), title: "some 'messy' title"),
        ],
        parentRenderElement: app!.appRenderElement,
      );
    ...
    test('should call ref with correct element', () async {
      ...
      await app!.buildChildren(
        widgets: [
          Variable(id: 'some-id-1', ref: (el) => el1 = el),
          Variable(id: 'some-id-2', ref: (el) => el2 = el),
          Variable(id: 'some-id-3', ref: (el) => el3 = el),
        ],
        parentRenderElement: app!.appRenderElement,
      );
    ...
    test('should set key', () async {
      await app!.buildChildren(
        widgets: [
          Variable(key: Key('some-key-1')),
          Variable(key: Key('some-key-2')),
          Variable(key: Key('some-key-3')),
        ],
        parentRenderElement: app!.appRenderElement,
      );
