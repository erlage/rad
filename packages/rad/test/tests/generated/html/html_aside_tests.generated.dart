...
void html_aside_test() {
  group('HTML Aside tests:', () {
    RT_AppRunner? app;
    ...
    test('should set id', () async {
      await app!.buildChildren(
        widgets: [
          Aside(key: Key('some-key-1'), id: 'some-id-1'),
          Aside(key: Key('some-key-2'), id: 'some-id-2'),
          Aside(key: Key('some-key-3'), id: 'some-id-3'),
        ],
    ...
    test('should clear attribute "classes" if updated value is null', () async {
      ...
      await app!.updateChildren(
        widgets: [
          Aside(key: Key('el-1'), className: null),
        ],
        updateType: UpdateType.setState,
        parentRenderElement: app!.appRenderElement,
      );
    ...
    test('should set attribute "hidden" only if its true', () async {
      await app!.buildChildren(
        widgets: [
          Aside(key: Key('el-1'), hidden: false),
          Aside(key: Key('el-2'), hidden: null),
          Aside(key: Key('el-3'), hidden: true),
        ],
        parentRenderElement: app!.appRenderElement,
      );
    ...
    test('should clear attribute "hidden" if updated value is not true',
      ...
      await app!.buildChildren(
        widgets: [
          Aside(key: Key('el-1'), hidden: true),
          Aside(key: Key('el-2'), hidden: true),
          Aside(key: Key('el-3'), hidden: true),
          Aside(key: Key('el-4'), hidden: true),
        ],
        parentRenderElement: app!.appRenderElement,
      );
      ...
      await app!.updateChildren(
        widgets: [
          Aside(key: Key('el-1'), hidden: true),
          Aside(key: Key('el-2'), hidden: false),
          Aside(key: Key('el-3'), hidden: null),
          Aside(key: Key('el-4')),
        ],
        updateType: UpdateType.setState,
        parentRenderElement: app!.appRenderElement,
    ...
    test('should set "click" event listener only if provided', () async {
      ...
      await app!.buildChildren(
        widgets: [
          Aside(key: Key('el-1')),
          Aside(key: Key('el-2'), onClick: null),
          Aside(key: Key('el-3'), onClick: listener),
        ],
        parentRenderElement: app!.appRenderElement,
      );
    ...
    test('should clear "click" event listener', () async {
      ...
      await app!.buildChildren(
        widgets: [
          Aside(key: Key('el-1')),
          Aside(key: Key('el-2'), onClick: listener),
        ],
        parentRenderElement: app!.appRenderElement,
      );
    ...
    test('should set style', () async {
      await app!.buildChildren(
        widgets: [
          Aside(key: Key('widget-1'), style: 'some style'),
          Aside(key: Key('widget-2'), style: 'some "messy" style'),
          Aside(key: Key('widget-3'), style: "some 'messy' style"),
        ],
        parentRenderElement: RT_TestBed.rootRenderElement,
      );
    ...
    test('should set title', () async {
      await app!.buildChildren(
        widgets: [
          Aside(key: Key('widget-1'), title: 'some title'),
          Aside(key: Key('widget-2'), title: 'some "messy" title'),
          Aside(key: Key('widget-3'), title: "some 'messy' title"),
        ],
        parentRenderElement: RT_TestBed.rootRenderElement,
      );
    ...
    test('should call ref with correct element', () async {
      ...
      await app!.buildChildren(
        widgets: [
          Aside(id: 'some-id-1', ref: (el) => el1 = el),
          Aside(id: 'some-id-2', ref: (el) => el2 = el),
          Aside(id: 'some-id-3', ref: (el) => el3 = el),
        ],
        parentRenderElement: app!.appRenderElement,
      );
    ...
    test('should set key', () async {
      await app!.buildChildren(
        widgets: [
          Aside(key: Key('some-key-1')),
          Aside(key: Key('some-key-2')),
          Aside(key: Key('some-key-3')),
        ],
        parentRenderElement: app!.appRenderElement,
      );
