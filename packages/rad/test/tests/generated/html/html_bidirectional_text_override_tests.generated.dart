...
void html_bidirectional_text_override_test() {
  group('HTML BidirectionalTextOverride tests:', () {
    RT_AppRunner? app;
    ...
    test('should set id', () async {
      await app!.buildChildren(
        widgets: [
          BidirectionalTextOverride(key: Key('some-key-1'), id: 'some-id-1'),
          BidirectionalTextOverride(key: Key('some-key-2'), id: 'some-id-2'),
          BidirectionalTextOverride(key: Key('some-key-3'), id: 'some-id-3'),
        ],
    ...
    test('should set child widget', () async {
      await app!.buildChildren(
        widgets: [
          BidirectionalTextOverride(
            id: 'widget-1',
            child: BidirectionalTextOverride(
              id: 'widget-2',
            ),
          ),
        ],
    ...
    test('should clear attribute "classes"', () async {
      await app!.buildChildren(
        widgets: [
          BidirectionalTextOverride(key: Key('el-1')),
          BidirectionalTextOverride(
            key: Key('el-2'),
            className: 'another-classes',
          ),
      ...
      await app!.updateChildren(
        widgets: [
          BidirectionalTextOverride(key: Key('el-1')),
          BidirectionalTextOverride(key: Key('el-2')),
        ],
        updateType: UpdateType.setState,
        parentRenderElement: app!.appRenderElement,
      );
    ...
    test('should clear attribute "classes" if updated value is null', () async {
      ...
      await app!.updateChildren(
        widgets: [
          BidirectionalTextOverride(key: Key('el-1'), className: null),
        ],
        updateType: UpdateType.setState,
        parentRenderElement: app!.appRenderElement,
      );
    ...
    test('should set attribute "hidden" only if its true', () async {
      await app!.buildChildren(
        widgets: [
          BidirectionalTextOverride(key: Key('el-1'), hidden: false),
          BidirectionalTextOverride(key: Key('el-2'), hidden: null),
          BidirectionalTextOverride(key: Key('el-3'), hidden: true),
        ],
        parentRenderElement: app!.appRenderElement,
      );
    ...
    test('should clear attribute "hidden" if updated value is not true',
      ...
      await app!.buildChildren(
        widgets: [
          BidirectionalTextOverride(key: Key('el-1'), hidden: true),
          BidirectionalTextOverride(key: Key('el-2'), hidden: true),
          BidirectionalTextOverride(key: Key('el-3'), hidden: true),
          BidirectionalTextOverride(key: Key('el-4'), hidden: true),
        ],
        parentRenderElement: app!.appRenderElement,
      );
      ...
      await app!.updateChildren(
        widgets: [
          BidirectionalTextOverride(key: Key('el-1'), hidden: true),
          BidirectionalTextOverride(key: Key('el-2'), hidden: false),
          BidirectionalTextOverride(key: Key('el-3'), hidden: null),
          BidirectionalTextOverride(key: Key('el-4')),
        ],
        updateType: UpdateType.setState,
        parentRenderElement: app!.appRenderElement,
      );
    ...
    test('should set "click" event listener only if provided', () async {
      ...
      await app!.buildChildren(
        widgets: [
          BidirectionalTextOverride(key: Key('el-1')),
          BidirectionalTextOverride(key: Key('el-2'), onClick: null),
          BidirectionalTextOverride(key: Key('el-3'), onClick: listener),
        ],
        parentRenderElement: app!.appRenderElement,
      );
    ...
    test('should clear "click" event listener', () async {
      ...
      await app!.buildChildren(
        widgets: [
          BidirectionalTextOverride(key: Key('el-1')),
          BidirectionalTextOverride(key: Key('el-2'), onClick: listener),
        ],
        parentRenderElement: app!.appRenderElement,
      );
    ...
    test('should set style', () async {
      await app!.buildChildren(
        widgets: [
          BidirectionalTextOverride(key: Key('widget-1'), style: 'some style'),
          BidirectionalTextOverride(
              key: Key('widget-2'), style: 'some "messy" style'),
          BidirectionalTextOverride(
    ...
    test('should set title', () async {
      await app!.buildChildren(
        widgets: [
          BidirectionalTextOverride(key: Key('widget-1'), title: 'some title'),
          BidirectionalTextOverride(
              key: Key('widget-2'), title: 'some "messy" title'),
          BidirectionalTextOverride(
    ...
    test('should call ref with correct element', () async {
      ...
      await app!.buildChildren(
        widgets: [
          BidirectionalTextOverride(id: 'some-id-1', ref: (el) => el1 = el),
          BidirectionalTextOverride(id: 'some-id-2', ref: (el) => el2 = el),
          BidirectionalTextOverride(id: 'some-id-3', ref: (el) => el3 = el),
        ],
        parentRenderElement: app!.appRenderElement,
      );
    ...
    test('should call ref-callback with null on dispose', () async {
      ...
      await app!.updateChildren(
        widgets: [BidirectionalTextOverride()],
        updateType: UpdateType.setState,
        parentRenderElement: app!.appRenderElement,
      );
    ...
    test('should not call ref-callback on update', () async {
      ...
      await app!.buildChildren(
        widgets: [
          BidirectionalTextOverride(ref: (el) {
            stack.push('callback');
          }),
        ],
        parentRenderElement: app!.appRenderElement,
    ...
    test('should have a short-tag alias', () async {
      var widget = BidirectionalTextOverride();
      var widgetShort = bdo();
    ...
    test('should set key', () async {
      await app!.buildChildren(
        widgets: [
          BidirectionalTextOverride(key: Key('some-key-1')),
          BidirectionalTextOverride(key: Key('some-key-2')),
          BidirectionalTextOverride(key: Key('some-key-3')),
        ],
        parentRenderElement: app!.appRenderElement,
      );
    ...
    test('should set attribute "dir"', () async {
      await app!.buildChildren(
        widgets: [
          ...
          BidirectionalTextOverride(key: Key('el-3'), dir: DirectionType.auto),
        ],
        parentRenderElement: app!.appRenderElement,
      );
    ...
    test('should update attribute "dir"', () async {
      ...
      await app!.updateChildren(
        widgets: [
          BidirectionalTextOverride(key: Key('el-1')),
          BidirectionalTextOverride(key: Key('el-2'), dir: null),
          BidirectionalTextOverride(
