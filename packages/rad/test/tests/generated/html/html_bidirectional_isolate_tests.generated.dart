...
void html_bidirectional_isolate_test() {
  group('HTML BidirectionalIsolate tests:', () {
    RT_AppRunner? app;
    ...
    test('should set id', () async {
      await app!.buildChildren(
        widgets: [
          BidirectionalIsolate(key: Key('some-key-1'), id: 'some-id-1'),
          BidirectionalIsolate(key: Key('some-key-2'), id: 'some-id-2'),
          BidirectionalIsolate(key: Key('some-key-3'), id: 'some-id-3'),
        ],
    ...
    test('should clear attribute "classes"', () async {
      ...
      await app!.updateChildren(
        widgets: [
          BidirectionalIsolate(key: Key('el-1')),
          BidirectionalIsolate(key: Key('el-2')),
        ],
        updateType: UpdateType.setState,
        parentRenderElement: app!.appRenderElement,
      );
    ...
    test('should clear attribute "classes" if updated value is null', () async {
      ...
      await app!.updateChildren(
        widgets: [
          BidirectionalIsolate(key: Key('el-1'), className: null),
        ],
        updateType: UpdateType.setState,
        parentRenderElement: app!.appRenderElement,
      );
    ...
    test('should set attribute "hidden" only if its true', () async {
      await app!.buildChildren(
        widgets: [
          BidirectionalIsolate(key: Key('el-1'), hidden: false),
          BidirectionalIsolate(key: Key('el-2'), hidden: null),
          BidirectionalIsolate(key: Key('el-3'), hidden: true),
        ],
        parentRenderElement: app!.appRenderElement,
      );
    ...
    test('should clear attribute "hidden" if updated value is not true',
      ...
      await app!.buildChildren(
        widgets: [
          BidirectionalIsolate(key: Key('el-1'), hidden: true),
          BidirectionalIsolate(key: Key('el-2'), hidden: true),
          BidirectionalIsolate(key: Key('el-3'), hidden: true),
          BidirectionalIsolate(key: Key('el-4'), hidden: true),
        ],
        parentRenderElement: app!.appRenderElement,
      );
      ...
      await app!.updateChildren(
        widgets: [
          BidirectionalIsolate(key: Key('el-1'), hidden: true),
          BidirectionalIsolate(key: Key('el-2'), hidden: false),
          BidirectionalIsolate(key: Key('el-3'), hidden: null),
          BidirectionalIsolate(key: Key('el-4')),
        ],
        updateType: UpdateType.setState,
        parentRenderElement: app!.appRenderElement,
      );
    ...
    test('should set "click" event listener only if provided', () async {
      ...
      await app!.buildChildren(
        widgets: [
          BidirectionalIsolate(key: Key('el-1')),
          BidirectionalIsolate(key: Key('el-2'), onClick: null),
          BidirectionalIsolate(key: Key('el-3'), onClick: listener),
        ],
        parentRenderElement: app!.appRenderElement,
      );
    ...
    test('should clear "click" event listener', () async {
      ...
      await app!.buildChildren(
        widgets: [
          BidirectionalIsolate(key: Key('el-1')),
          BidirectionalIsolate(key: Key('el-2'), onClick: listener),
        ],
        parentRenderElement: app!.appRenderElement,
      );
    ...
    test('should set style', () async {
      await app!.buildChildren(
        widgets: [
          BidirectionalIsolate(key: Key('widget-1'), style: 'some style'),
          BidirectionalIsolate(
    ...
    test('should set title', () async {
      await app!.buildChildren(
        widgets: [
          BidirectionalIsolate(key: Key('widget-1'), title: 'some title'),
          BidirectionalIsolate(
    ...
    test('should call ref with correct element', () async {
      ...
      await app!.buildChildren(
        widgets: [
          BidirectionalIsolate(id: 'some-id-1', ref: (el) => el1 = el),
          BidirectionalIsolate(id: 'some-id-2', ref: (el) => el2 = el),
          BidirectionalIsolate(id: 'some-id-3', ref: (el) => el3 = el),
        ],
        parentRenderElement: app!.appRenderElement,
      );
    ...
    test('should set correct types and markup', () async {
      await app!.buildChildren(
        widgets: [
          BidirectionalIsolate(key: Key('some-key-3')),
        ],
        parentRenderElement: RT_TestBed.rootRenderElement,
      );
    ...
    test('should have a short-tag alias', () async {
      var widget = BidirectionalIsolate();
      var widgetShort = bdi();
    ...
    test('should set key', () async {
      await app!.buildChildren(
        widgets: [
          BidirectionalIsolate(key: Key('some-key-1')),
          BidirectionalIsolate(key: Key('some-key-2')),
          BidirectionalIsolate(key: Key('some-key-3')),
        ],
        parentRenderElement: app!.appRenderElement,
      );
