...
void html_field_set_test() {
  group('HTML FieldSet tests:', () {
    RT_AppRunner? app;
    ...
    test('should set id', () async {
      await app!.buildChildren(
        widgets: [
          FieldSet(key: Key('some-key-1'), id: 'some-id-1'),
          FieldSet(key: Key('some-key-2'), id: 'some-id-2'),
          FieldSet(key: Key('some-key-3'), id: 'some-id-3'),
        ],
    ...
    test('should clear attribute "classes" if updated value is null', () async {
      ...
      await app!.updateChildren(
        widgets: [
          FieldSet(key: Key('el-1'), className: null),
        ],
        updateType: UpdateType.setState,
        parentRenderElement: app!.appRenderElement,
      );
    ...
    test('should set attribute "hidden" only if its true', () async {
      await app!.buildChildren(
        widgets: [
          FieldSet(key: Key('el-1'), hidden: false),
          FieldSet(key: Key('el-2'), hidden: null),
          FieldSet(key: Key('el-3'), hidden: true),
        ],
        parentRenderElement: app!.appRenderElement,
      );
    ...
    test('should clear attribute "hidden" if updated value is not true',
      ...
      await app!.buildChildren(
        widgets: [
          FieldSet(key: Key('el-1'), hidden: true),
          FieldSet(key: Key('el-2'), hidden: true),
          FieldSet(key: Key('el-3'), hidden: true),
          FieldSet(key: Key('el-4'), hidden: true),
        ],
        parentRenderElement: app!.appRenderElement,
      );
      ...
      await app!.updateChildren(
        widgets: [
          FieldSet(key: Key('el-1'), hidden: true),
          FieldSet(key: Key('el-2'), hidden: false),
          FieldSet(key: Key('el-3'), hidden: null),
          FieldSet(key: Key('el-4')),
        ],
        updateType: UpdateType.setState,
        parentRenderElement: app!.appRenderElement,
      );
    ...
    test('should set "click" event listener only if provided', () async {
      ...
      await app!.buildChildren(
        widgets: [
          FieldSet(key: Key('el-1')),
          FieldSet(key: Key('el-2'), onClick: null),
          FieldSet(key: Key('el-3'), onClick: listener),
        ],
        parentRenderElement: app!.appRenderElement,
      );
    ...
    test('should clear "click" event listener', () async {
      ...
      await app!.buildChildren(
        widgets: [
          FieldSet(key: Key('el-1')),
          FieldSet(key: Key('el-2'), onClick: listener),
        ],
        parentRenderElement: app!.appRenderElement,
      );
    ...
    test('should set style', () async {
      await app!.buildChildren(
        widgets: [
          FieldSet(key: Key('widget-1'), style: 'some style'),
          FieldSet(key: Key('widget-2'), style: 'some "messy" style'),
          FieldSet(key: Key('widget-3'), style: "some 'messy' style"),
        ],
        parentRenderElement: RT_TestBed.rootRenderElement,
      );
    ...
    test('should set title', () async {
      await app!.buildChildren(
        widgets: [
          FieldSet(key: Key('widget-1'), title: 'some title'),
          FieldSet(key: Key('widget-2'), title: 'some "messy" title'),
          FieldSet(key: Key('widget-3'), title: "some 'messy' title"),
        ],
        parentRenderElement: RT_TestBed.rootRenderElement,
      );
    ...
    test('should call ref with correct element', () async {
      ...
      await app!.buildChildren(
        widgets: [
          FieldSet(id: 'some-id-1', ref: (el) => el1 = el),
          FieldSet(id: 'some-id-2', ref: (el) => el2 = el),
          FieldSet(id: 'some-id-3', ref: (el) => el3 = el),
        ],
        parentRenderElement: app!.appRenderElement,
      );
    ...
    test('should set key', () async {
      await app!.buildChildren(
        widgets: [
          FieldSet(key: Key('some-key-1')),
          FieldSet(key: Key('some-key-2')),
          FieldSet(key: Key('some-key-3')),
        ],
        parentRenderElement: app!.appRenderElement,
      );
    ...
    test('should set attribute "name"', () async {
      await app!.buildChildren(
        widgets: [
          FieldSet(key: Key('el-1'), name: 'some-name'),
          FieldSet(key: Key('el-2'), name: 'another-name'),
        ],
        parentRenderElement: app!.appRenderElement,
      );
    ...
    test('should update attribute "name"', () async {
      ...
      await app!.updateChildren(
        widgets: [
          FieldSet(key: Key('el-1'), name: 'updated-name'),
          FieldSet(key: Key('el-2'), name: 'another-name'),
        ],
        updateType: UpdateType.setState,
        parentRenderElement: app!.appRenderElement,
    ...
    test('should clear attribute "name" if updated value is null', () async {
      ...
      await app!.updateChildren(
        widgets: [
          FieldSet(key: Key('el-1'), name: null),
        ],
        updateType: UpdateType.setState,
        parentRenderElement: app!.appRenderElement,
      );
    ...
    test('should set attribute "form"', () async {
      await app!.buildChildren(
        widgets: [
          FieldSet(key: Key('el-1'), form: 'some-form'),
          FieldSet(key: Key('el-2'), form: 'another-form'),
        ],
        parentRenderElement: app!.appRenderElement,
      );
    ...
    test('should update attribute "form"', () async {
      ...
      await app!.updateChildren(
        widgets: [
          FieldSet(key: Key('el-1'), form: 'updated-form'),
          FieldSet(key: Key('el-2'), form: 'another-form'),
        ],
        updateType: UpdateType.setState,
        parentRenderElement: app!.appRenderElement,
    ...
    test('should clear attribute "form" if updated value is null', () async {
      ...
      await app!.updateChildren(
        widgets: [
          FieldSet(key: Key('el-1'), form: null),
        ],
        updateType: UpdateType.setState,
        parentRenderElement: app!.appRenderElement,
      );
    ...
    test('should set attribute "disabled" only if its true', () async {
      await app!.buildChildren(
        widgets: [
          FieldSet(key: Key('el-1'), disabled: false),
          FieldSet(key: Key('el-2'), disabled: null),
          FieldSet(key: Key('el-3'), disabled: true),
        ],
        parentRenderElement: app!.appRenderElement,
      );
    ...
    test('should clear attribute "disabled" if updated value is not true',
      ...
      await app!.buildChildren(
        widgets: [
          FieldSet(key: Key('el-1'), disabled: true),
          FieldSet(key: Key('el-2'), disabled: true),
          FieldSet(key: Key('el-3'), disabled: true),
          FieldSet(key: Key('el-4'), disabled: true),
        ],
        parentRenderElement: app!.appRenderElement,
      );
      ...
      await app!.updateChildren(
        widgets: [
          FieldSet(key: Key('el-1'), disabled: true),
          FieldSet(key: Key('el-2'), disabled: false),
          FieldSet(key: Key('el-3'), disabled: null),
          FieldSet(key: Key('el-4')),
        ],
        updateType: UpdateType.setState,
        parentRenderElement: app!.appRenderElement,
