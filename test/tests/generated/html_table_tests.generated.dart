...
void html_table_test() {
  group('HTML Table tests:', () {
    ...
    test('should set children widgets', () {
      app!.framework.buildChildren(
        widgets: [
          Table(key: GlobalKey('widget-1'), children: [
            Table(
              key: GlobalKey('widget-2'),
            ),
            Table(
    ...
    test('should set correct types and markup', () {
      app!.framework.buildChildren(
        widgets: [
          Table(key: GlobalKey('some-global-key')),
        ],
        parentContext: RT_TestBed.rootContext,
      );
      ...
      expect(
        ...
        startsWith(
          '<table id="some-global-key" '
          'data-${Constants.attrWidgetType}="$Table" '
          'data-${Constants.attrRuntimeType}="$Table">',
    ...
    test('should not override system reserved data attributes on build', () {
      ...
      expect(element1.dataset[Constants.attrWidgetType], equals("$Table"));
      expect(element1.dataset[Constants.attrRuntimeType], equals("$Table"));
    ...
    test('should set key', () {
      app!.framework.buildChildren(
        widgets: [
          Table(key: Key('some-key')),
          Table(key: LocalKey('some-local-key')),
          Table(key: GlobalKey('some-global-key')),
        ],
        parentContext: RT_TestBed.rootContext,
      );
